...
- 由 VPS 异步检测网站是否可访问，显示 HTTP 状态与分级延迟
- 单次失败会在 1 秒、2 秒后复核，连续 3 次失败才标记异常并触发通知
- VPS 后台默认每 10 分钟自动检测，可在设置中自定义 1 分钟到 24 小时的间隔
- 网站首次异常、由正常转异常或恢复时，可通过 Telegram Bot 和 SMTP 邮件通知
...
- Telegram Token 与 SMTP 密码使用 `SESSION_SECRET` 加密后保存，不会通过设置接口回传
...
- 数据文件最大 4 MiB
- 页面每次最多渲染 96 条，并跳过屏幕外卡片的绘制，避免大书签库一次性撑满 DOM
- 网站检测固定为 6 个并发，复用 HTTP 连接；先轻量探测，失败后再做两次兼容复核
- 每次网络请求最多等待 10 秒，复核仍占用原有工作位，不会额外扩张协程和连接数
- 无网络壁纸、无远程图标请求；自动检测只保留一个调度计时器
